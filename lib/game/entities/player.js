...
EntityPlayer = ig.Entity.extend({
	...
	update: function() {
		...
		if( ig.input.state('left') ) {
			...
			this.vel.x = this.accelGround;
		} else if( Math.abs(axesX = ig.gamepadInput.axis(this.controllerNumber, ig.AXES.LEFT_X)) > 0.5 ) {
			this.vel.x = this.accelGround * axesX;
		} else {
			this.vel.x = 0;
		}
		...
		if( ig.input.state('up') ) {
			...
			this.vel.y = this.accelGround;
			this.currentAnim.angle = Math.PI * 0.5;
		} else if( Math.abs(axesY = ig.gamepadInput.axis(this.controllerNumber, ig.AXES.LEFT_Y)) > 0.5 ) {
			this.vel.y = this.accelGround * axesY;
		} else {
			this.vel.y = 0;
		}
