...
ig.PlayerManager = ig.Class.extend({
  ...
  updateActions: function() {
    var i = this.players.length, player,
        shouldAttack, shouldInteract, shouldShoot,
        enemies = ig.game.getEntitiesByType( EntityEnemy ),
        enemyIndex, useKeyboard = false;

    while( i-- ) {
      player = this.players[i];
      if( !player ) continue;
      if( this.useKeyboard && i === 0 ) useKeyboard = true;

      // handle attacking
      shouldAttack = false;
      if( ig.gamepadInput.pressed( i, ig.BUTTON.RIGHT_BUTTON ) ) shouldAttack = true;
      if( useKeyboard && ig.input.pressed( 'attack' ) ) shouldAttack = true;
      ...
      if( player.isAttacking ) {
        ...
        while( enemyIndex-- )
          enemies[enemyIndex].checkAttack( player );
      }

      // handle shooting
      shouldShoot = false;
      if( ig.gamepadInput.pressed( i, ig.BUTTON.LEFT_BUTTON ) ) shouldShoot = true;
      if( useKeyboard && ig.input.pressed( 'shoot' ) ) shouldShoot = true;
      ...
      if( useKeyboard && ig.input.pressed( 'interact' ) ) shouldInteract = true;
